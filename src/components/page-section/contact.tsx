...
function ContactPage() {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(process.env.EMAILJS_SERVICE_ID as string, process.env.EMAILJS_TEMPLATE_ID as string, form.current as HTMLFormElement, {
    ...
    <div>
      <div className="h-full w-full flex flex-col justify-center items-center gap-3 mt-4 md:mt-8">
        ...
        <div className='w-full max-w-md p-4 bg-slate-900 rounded-2xl shadow-md'>
          <form className='flex flex-col gap-2' name='form' ref={form} onSubmit={sendEmail}>
            <div>
              <input type="text" id="name" name="name" placeholder='Name' className='bg-slate-950 w-full py-2 px-2 rounded-[12px]' required />
            </div>
            <div>
              <input type="email" id="email" name="email" placeholder='Email' className='bg-slate-950 w-full py-2 px-2 rounded-[12px]' required />
            </div>
            <div>
              <textarea id="message" name="message" placeholder='Message' className='bg-slate-950 w-full py-2 px-2 h-24 rounded-[12px]' required></textarea>
            </div>
